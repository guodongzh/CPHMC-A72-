...
*@file       net_port_j721e.c
*@author     xuesen
*@date       2026.07.08
*@brief      J721E 平台网络协议适配实现。
*@par        History
...
/**
 * @brief 登记J721E平台当前选中的本机MAC和IP地址。
 * @param mac 本机MAC地址
 * @param ip 本机IP地址
...
/**
 * @brief 获取J721E平台最近登记的本机MAC和IP地址。
...
/**
 * @brief 根据接收端口加载J721E本地网口的MAC和IP信息。
...
bool net_port_get_current_enet_info(NET_RAW_PKG_INFO *pInfo)
{
    ...
    current_inf.Slot_ID  = pInfo->Src_Slot_ID;
...
/**
 * @brief 根据接收端口加载J721E组网网口的MAC和IP信息。
 * @param pInfo 接收报文及来源端口信息
 * @return true表示加载成功，false表示参数或配置无效
 */
                          ...
                          const net_send_route_t *pRoute)
{
    ...
    return sendto_raw_socket(0U, METH_UDP_IP_SOCKET_ID, frame, len, 0, port_def, dst_mac, htons(eth_type), 0U);
}

/**
 * @brief 通过J721E组网网口发送以太网协议载荷。
 * @param frame 以太网类型字段后的协议载荷
 * @param len 协议载荷长度
 * @param dst_mac 目的MAC地址
 * @param eth_type 以太网类型，使用主机字节序
                                 ...
                                 const net_send_route_t *pRoute)
{
    ...
    net_port_route_to_enet(pRoute, &port_def);
    return sendto_networking_socket(0U, METH_UDP_IP_SOCKET_ID, frame, len, 0, port_def, dst_mac, htons(eth_type), 0U);
}

/**
 * @brief 保留J721E协议栈调试日志接口。
 * @param fmt 格式化字符串
 */
void net_port_debug_log(const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    (void)vsnprintf(log_buffer, sizeof(log_buffer), fmt, args);
    va_end(args);

    // 统一经DebugP输出到当前工程配置的调试串口。
    DebugP_log("%s", log_buffer);
#else
    (void)fmt;
#endif
}
